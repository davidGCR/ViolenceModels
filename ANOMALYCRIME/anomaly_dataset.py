...
class AnomalyDataset(Dataset):
    ...
    def skip_initial_segments(self, percentage, video_segments):
        cut = int((len(video_segments)*percentage)/100) 
        # print('Trimed sequence: ', len(sequences), cut)
        video_segments = video_segments[cut:]
        return video_segments, cut

    def getRandomSegment(self, video_segments, idx): #only if numDiPerVideo == 1
        random_segment = None
        ...
        if self.numFrames[idx] > self.videoSegmentLength:
            if label == 0: #Normal videos
                video_segments, _ = self.skip_initial_segments(self.skipPercentage,video_segments) #skip 35% of initial segments
    ...
    def getCentralSegment(self, sequences, idx): #only if numDiPerVideo == 1
        segment = None
    ...
    def getVideoSegments(self, vid_name, idx):
        ...
        if self.numDynamicImagesPerVideo == 1 and self.videoSegmentLength > 0:
            ...
            random_segment = self.getRandomSegment(video_segments, idx) if self.positionSegment == 'random' else self.getCentralSegment(video_segments,idx)
            video_segments = []
            video_segments.append(random_segment)
        else:
